...
;; That's a concise Hello World for any language. And note that there are no parens. 😀

;; This guide will try to give you a basic understanding of the Clojure language. Basic in the sense that it is not extensive. Basic in the
;; sense that it is foundational, building from first principles in order to make the Clojure journey you have ahead easier to comprehend.

;; With the foundations in place you'll have a good chance of having the right gut feeling for how to code something, how to formulate your questions,
;; how to search effectively for information, how to make sense of code you stumble across, and so on.

;; There will be links here and there, ctrl/cmd-click;; those to open them in a browser. Here's the first;; such link; 
;; https://clojure.org/guides/learn/syntax
;; There you can read more about the concepts;; mentioned in this guide.

;; The way to use the guide is to read about the;; concepts and evaluate the examples. Sometimes there ;; will be exercises in the text. Don't limit your
;; exercising to those, though. Please feel encouraged;; to edit the examples, and add new code and evaluate that. Evaluate this to warm up:
...
;; Then see what happens if you throw in some numbers here and there and evaluate again.

;; NB: This is work in progress...;; When you fire up the Getting Started REPL the next;; time, you will be presented with the option to;; download new files or 
;;continue with the files you have. You can also always find the latest version here:
;; https://github.com/BetterThanTomorrow/dram/blob/dev/drams/calva_getting_started/welcome_to_clojure.clj

(comment
  ;; = EXPRESSIONS =
  ;; In Clojure everything is an expression.;; (There are no statements.) Unless there is an error when evaluating an expression, there is always a return value 
  ;;(which is sometimes `nil`).

  ;; An important aspect of this is that the result of an expression is always the last form/expression  evaluated. E.g. if you have a function defined like so:
  ...
  ;; This defines a function named `last-eval-wins`, taking no arguments, with four expressions in its function body. (We'll return to defining functions later.)

  ;; Calling the function

  (last-eval-wins) ; <- Evaluate that 😄

  ;; will cause all four expressions in the function body to be evaluated. The result of the call will be the last expression that was evaluated.

  ;; In the output window you will also see the `println` calls happening. They are also expressions, evaluating to `nil`.

  (println 'prints-this-evaluates-to-nil)

  ;; Expressions are composed from literals (evaluating to themselves) and/or calls to either:
  ;; * special forms
  ;; * macros
  ;; * functions

  ;; ”Hello World” at the beginning of this guide is a literal string (thus, it evaluates to itself). More about literals in the next section.

  ;; Calls are written as lists with the called thing as the first element.

  (def foo "foo") ; Calls the special form `def`, evaluates to the var it creates (More on this later)

  (for [x '(1 2 3)  ; Calls the macro `for`
        y '(:a :b)] ; (List comprehension)
    [x y])

  (str 1 2 3) ; Calls the function `str` with the arguments 1, 2, and 3.
...
(comment
  ;; = LITERALS =
  ;; Literals evaluate to themselves. (Remember your friends: Alt+Enter and Ctrl+Enter)
  ...
  ::alpha         ; namespaced keyword, in current namespace

  ;; == KEYWORDS ==
  ;; Keywords start with a `:`. They are a thing in themselves, often used as identifiers and as keys in maps (more on maps later). Keywords are very memory and speed 
  ;; efficient. 
  ...
  ;; This means it is the same thing, occupying the same (very tiny) place in memory. Even if you construct a non-literal keyword it remains identical to its literal 
  ;; form

  (identical? (keyword "foo") :foo)

  ;; This holds true for your whole Clojure program. Keywords are global. There is namespace syntax for them, so that you can have control of this.

  ;; Keywords are also functions, actually. But more on that later. For now let it suffice to say that keywords have a very special and important role in most Clojure   
  ;; programs.
